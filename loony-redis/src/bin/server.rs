...
use loony_redis::{server, DEFAULT_PORT};
...
#[tokio::main]
pub async fn main() -> loony_redis::Result<()> {
    set_up_logging()?;
...
#[derive(Parser, Debug)]
#[command(name = "loony-redis-server", version, author, about = "A Redis server")]
struct Cli {
    #[arg(long)]
    port: Option<u16>,
}
...
fn set_up_logging() -> loony_redis::Result<()> {
